    ...
    <div class="profile-layout">
      ...
      <el-card class="orders-card" shadow="never">
        ...
        <el-skeleton :loading="ordersLoading" animated>
          ...
          <template><div v-if="orders.length" class="order-list"><article v-for="order in orders" :key="order.id" class="order-item"><div><p class="order-sn">订单号：{{ order.orderSn || order.id }}</p><p>{{ order.productName || '积分兑换商品' }}</p><time>{{ order.createTime }}</time></div><div class="order-summary"><strong>{{ order.costPoints }} 积分</strong><el-tag size="small" :type="statusType(order.status)">{{ order.status }}</el-tag><div class="order-actions"><el-button v-if="order.status === '待发货'" size="mini" @click="applyCancel(order)">申请取消</el-button><el-button v-if="['已发货', '已完成'].includes(order.status)" type="primary" plain size="mini" @click="showLogistics(order)">查看物流</el-button></div></div></article></div><el-empty v-else description="暂无相关订单" :image-size="100" /></template>
        </el-skeleton>
      </el-card>
    </div>
    <el-dialog title="物流信息" :visible.sync="logisticsVisible" width="520px"><p v-if="logisticsOrder" class="dialog-order">订单号：{{ logisticsOrder.orderSn || logisticsOrder.id }}</p><el-timeline v-if="logisticsTracks.length"><el-timeline-item v-for="track in logisticsTracks" :key="track.id" :timestamp="track.eventTime">{{ track.trackStatus }}：{{ track.description }}<span v-if="track.location">（{{ track.location }}）</span></el-timeline-item></el-timeline><el-empty v-else description="暂未获取到物流轨迹" :image-size="80" /></el-dialog>
  </section>
</template>
...
export default {
  data() { return { userInfo: defaultUser(), form: defaultUser(), profileLoading: true, ordersLoading: true, saving: false, editing: false, tagInput: '', serverErrors: {}, activeTab: 'all', orders: [], orderRefreshTimer: null, logisticsVisible: false, logisticsTracks: [], logisticsOrder: null, tabs: [{ value: 'all', label: '全部订单' }, { value: 'completed', label: '已完成' }, { value: 'unfinished', label: '未完成' }, { value: 'deleted', label: '已删除' }] } },
  computed: { sexText() { return ({ 0: '女', 1: '男', 2: '保密' })[this.userInfo.sex] || '保密' } },
  created() { this.fetchProfile(); this.fetchOrders(); this.orderRefreshTimer = window.setInterval(this.fetchOrders, 15000); window.addEventListener('focus', this.fetchOrders) },
  beforeDestroy() { if (this.orderRefreshTimer) window.clearInterval(this.orderRefreshTimer); window.removeEventListener('focus', this.fetchOrders) },
  methods: {
